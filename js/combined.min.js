...
var hasRunCallback=!1,callbBackEnded=!1,responses={loaded:function(){var eventName=hasRunCallback?'ss.onProgressEnd':'ss.onStartEnd';if(!callbBackEnded||!hasRunCallback){$container.one(eventName,function(){updateContent(settings.url);if(!cacheResponse){clear(settings.url)}})}
...
$("#notification").html(output);$(".notification").delay(15000).queue(function(next){$(this).addClass("scale-out");next()});$(".notification").on("click",function(){$(this).addClass("scale-out")})},'json')});$(".magnific-popup, a[data-rel^='magnific-popup']").magnificPopup({type:'image',mainClass:'mfp-with-zoom',zoom:{enabled:!0,duration:300,easing:'ease-in-out',fixedContentPos:!0,opener:function(openerElement){return openerElement.is('img')?openerElement:openerElement.find('img')}},retina:{ratio:1,replaceSrc:function(item,ratio){return item.src.replace(/\.\w+$/,function(m){return '@2x'+m})}}});$('.image-gallery').magnificPopup({delegate:'a',type:'image',fixedContentPos:!0,gallery:{enabled:!0},removalDelay:300,mainClass:'mfp-fade',retina:{ratio:1,replaceSrc:function(item,ratio){return item.src.replace(/\.\w+$/,function(m){return '@2x'+m})}}});$('.popup-youtube, .popup-vimeo, .popup-gmaps').magnificPopup({type:'iframe',mainClass:'mfp-fade',removalDelay:300,preloader:!1,fixedContentPos:!0,});$('[data-toggle="popover"]').popover();$('[data-toggle="tooltip"]').tooltip({animated:'fade',container:'body'});function initialize(){var delayMillis=50;setTimeout(function(){var map;var panorama;var var_latitude=59.417736;var var_longitude=17.918668;var pin='images/icons/pin.svg';var title="Ett Smart - Click to see";var hotel_name="Ett Smart";var hotel_address="Sollentunavägen - Stockholm";var hotel_desc="Smart och lyxigt hotell";var hotel_more_desc="Ett hotell nära stockholm.";var hotel_location=new google.maps.LatLng(var_latitude,var_longitude);var mapOptions={center:hotel_location,zoom:15,scrollwheel:!1,streetViewControl:!1};map=new google.maps.Map(document.getElementById('map-canvas'),mapOptions);var contentString='<div id="infowindow_content">'+'<p><strong>'+hotel_name+'</strong><br>'+hotel_address+'<br>'+hotel_desc+'<br>'+hotel_more_desc+'</p>'+'</div>';var var_infowindow=new google.maps.InfoWindow({content:contentString});var marker=new google.maps.Marker({position:hotel_location,map:map,icon:pin,title:title,maxWidth:500,optimized:!1,});google.maps.event.addListener(marker,'click',function(){var_infowindow.open(map,marker)});panorama=map.getStreetView();panorama.setPosition(hotel_location);panorama.setPov(({heading:265,pitch:0}));var openStreet=document.getElementById('openStreetView');if(openStreet){document.getElementById("openStreetView").onclick=function(){toggleStreetView()}}
function toggleStreetView(){var toggle=panorama.getVisible();if(toggle==!1){panorama.setVisible(!0)}else{panorama.setVisible(!1)}}
google.maps.event.addDomListener(window,'resize',function(){var center=map.getCenter();map.setCenter(center)});google.maps.event.trigger(map,"resize")},delayMillis)}
if($("#map-canvas").length>0){google.maps.event.addDomListener(window,'load',initialize)}
var amountScrolled=500;var back_to_top=$('#back_to_top');$(window).on('scroll',function(){if($(window).scrollTop()>amountScrolled){back_to_top.addClass('active')}else{back_to_top.removeClass('active')}});back_to_top.on('click',function(){$('html, body').animate({scrollTop:0},500);return!1})})})(jQuery)